...
education = st.selectbox("Education Level",[1,2,3,4],help = '1=Primary School, 2 = High School, 3=College, 4=UG')
CurrentSmoker = st.selectbox('CurrentSmoker',[0,1],format_func=lambda x : 'No' if x == 0 else 'Yes' )
...
if st.button('Predict'):
    ...
    probability = model.predict_proba(input_scaled)[0][1] # It contain in the 2D array

    st.markdown('-----')

    if prediction[0] == 1:
        st.error("High Risk of Coranary Heart Disease")
        st.snow()
    else:
        st.success("Low Risk of Coranary Heart Disease")
        st.balloons()
